...
class Npc:
    def __init__(self, IterationName, MapSave):
        try:
    ...
    def MoveNpc(self, MapSave, PlayerLocation):
    ...
    def Collide(self,Movement_Angle):
        if Movement_Angle == "Up": 
            self.NpcPosition[0]+=1 # Moves npc back down 1 place in the y axis
            return self.NpcPosition
        if Movement_Angle == "Left":
            self.NpcPosition[1]+=1 # Moves npc back down 1 place in the x axis               
            return self.NpcPosition
        if Movement_Angle == "Down":
            self.NpcPosition[0] -= 1
            return self.NpcPosition
